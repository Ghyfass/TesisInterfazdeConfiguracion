...
export class OrganismosComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.context.getOrganismos().subscribe(data => {
  ...
  crear() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result && result.nombre.trim() !== '') {
        ...
        this.context.createOrganismo(result);
        this.snackBar.open('Organismo creado', 'Cerrar', { duration: 2500 });
      }
    });
  }
  ...
  editar(item: any) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.context.updateOrganismo(result);
        this.snackBar.open('Organismo actualizado', 'Cerrar', { duration: 2500 });
      }
    });
  }
  ...
  eliminar(item: any) {
    ...
    dialogRef.afterClosed().subscribe(confirmado => {
      if (confirmado) {
        this.context.deleteOrganismo(item.id);
        this.snackBar.open('Organismo eliminado', 'Cerrar', { duration: 2500 });
      }
    });
  }

  toggleEstado(item: any) {
    this.context.toggleEstadoOrganismo(item.id);
